...
mod credential_management {
    ...
    async fn test_router_clone_preserves_bandit_state() {
        ...
        let s = stats.expect("valid result should be retrieved");
        assert_eq!(s.pulls, 1);
...
mod plan_operations {
    ...
    async fn test_route_plan_item_fields() {
        ...
        assert!(plan.primary.is_some());
        let primary = plan.primary.expect("valid result should be retrieved");
...
mod edge_cases {
    ...
    async fn test_router_record_result_updates_all_subsystems() {
        ...
        let m = metrics.expect("valid result should be retrieved");
