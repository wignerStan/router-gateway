...
mod basic_get_and_cache {
    ...
    async fn test_registry_get_multiple_empty() {
        ...
        assert!(result.is_ok());
        assert_eq!(result.expect("valid result should be retrieved").len(), 0);
    }
    ...
    async fn test_registry_get_multiple_with_empty_strings() {
        ...
        assert!(result.is_ok());
        let models = result.expect("valid result should be retrieved");
...
mod cache_management {
    ...
    async fn test_registry_concurrent_access() {
        ...
        for handle in handles {
            let result = handle.await.expect("valid result should be retrieved");
            assert!(result.is_ok());
        }
...
mod find_best_fit {
    ...
    async fn test_find_best_fit_multiple_same_cost() {
        ...
        let model = best.expect("valid result should be retrieved");
