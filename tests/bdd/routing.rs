...
use gateway::routing::classification::{ContentTypeDetector, TokenEstimator};
...
async fn given_high_success_candidate(world: &mut BddWorld, success_pct: u64) {
    ...
    // Record with high utility to reflect high success rate
    for _ in 0..20 {
        world
            .bandit_policy
...
// -- Given: bandit exploration/exploitation scenarios --

#[given("multiple feasible route candidates")]
async fn given_multiple_feasible(world: &mut BddWorld) {
    world.bandit_routes = vec![
        "route-alpha".to_string(),
        "route-beta".to_string(),
        "route-gamma".to_string(),
    ];
    // Record some data on each
    world.bandit_policy.record_result("route-alpha", true, 0.7);
    world.bandit_policy.record_result("route-beta", true, 0.6);
    world.bandit_policy.record_result("route-gamma", true, 0.5);
}

#[given("limited historical data on some routes")]
async fn given_limited_data(world: &mut BddWorld) {
    // route-alpha has data, route-beta has none (will use prior)
    world.bandit_policy.record_result("route-alpha", true, 0.8);
}
...
async fn given_one_high_success(world: &mut BddWorld) {
    world.bandit_routes = vec![
        ...
        "mediocre-route".to_string(),
        "weak-route".to_string(),
    ];
    // Record many high-utility results on best-route
    for _ in 0..20 {
        world.bandit_policy.record_result("best-route", true, 0.95);
    }
    // Record mediocre results on others
    for _ in 0..5 {
        world
            .bandit_policy
            .record_result("mediocre-route", true, 0.5);
        world.bandit_policy.record_result("weak-route", false, 0.2);
    }
}
...
async fn given_same_provider_candidates(world: &mut BddWorld) {
    world.bandit_routes = vec![
        "openai-gpt4-auth1".to_string(),
        "openai-gpt4-auth2".to_string(),
        "anthropic-claude-auth1".to_string(),
    ];
    for route in &world.bandit_routes {
        world.bandit_policy.record_result(route, true, 0.7);
    }
}

#[given("only two feasible route candidates")]
async fn given_only_two_candidates(world: &mut BddWorld) {
    world.bandit_routes = vec!["route-primary".to_string(), "route-fallback".to_string()];
    world
        .bandit_policy
        .record_result("route-primary", true, 0.8);
    world
        .bandit_policy
        .record_result("route-fallback", true, 0.6);
}

#[given("multiple candidates for same provider")]
async fn given_same_provider_multiple(world: &mut BddWorld) {
    world.bandit_routes = vec![
        "openai-auth-aaa".to_string(),
        "openai-auth-bbb".to_string(),
        "openai-auth-ccc".to_string(),
    ];
    for route in &world.bandit_routes {
        world.bandit_policy.record_result(route, true, 0.7);
    }
...
async fn when_route_selected(world: &mut BddWorld) {
    let route_refs: Vec<&str> = world
        .bandit_routes
        .iter()
        .map(std::string::String::as_str)
        .collect();
    if let Some(selected) = world.bandit_policy.select_route(&route_refs) {
        world.current_auth_id = selected;
    }
}

#[when("primary and fallback are selected")]
async fn when_primary_fallback_selected(world: &mut BddWorld) {
    let route_refs: Vec<&str> = world
        .bandit_routes
        .iter()
        .map(std::string::String::as_str)
...
async fn when_route_decision_made(world: &mut BddWorld) {
    let route_refs: Vec<&str> = world
        .bandit_routes
        .iter()
        .map(std::string::String::as_str)
...
async fn when_fallback_planned(world: &mut BddWorld) {
    // Build fallback list from remaining routes after primary selection
    let route_refs: Vec<&str> = world
        .bandit_routes
        .iter()
        .map(std::string::String::as_str)
        .collect();
    if let Some(primary) = world.bandit_policy.select_route(&route_refs) {
        world.current_auth_id = primary;
    }
}

#[when("the next route is planned")]
async fn when_next_route_planned(world: &mut BddWorld) {
    // For session affinity scenarios, the existing provider affinity drives selection.
    // Ensure there are routes available for the bandit to select from.
    if world.bandit_routes.is_empty() {
        // Use the session's preferred provider as the basis for route IDs
        let provider = world
            .session_manager
    ...
    let route_refs: Vec<&str> = world
        ...
        .map(std::string::String::as_str)
        .collect();
    // Select a route — session affinity ensures the preferred provider gets bonus
    let _ = world.bandit_policy.select_route(&route_refs);
    // Preserve the existing session affinity (don't override preferred_provider)
    // The session already has 5 turns recorded, which provides the selection bonus.
}
...
async fn then_utility_high(world: &mut BddWorld) {
    // Verify the route with 95% success has high utility (last_utility close to success rate)
    let route = world
        .bandit_routes
        .iter()
        .find(|r| r.contains("95pct"))
        .expect("95% success route should exist");
    let stats = world
        .bandit_policy
        .get_stats(route)
    ...
    assert!(
        ...
        "utility should be high for 95% success route (got {})",
        stats.last_utility,
    );
}

#[then("the utility score should be reduced")]
async fn then_utility_reduced(world: &mut BddWorld) {
    // Verify the high-latency route has reduced utility
    let route = world
        .bandit_routes
        .iter()
        .find(|r| r.contains("5000ms"))
        .expect("high latency route should exist");
    let stats = world
        .bandit_policy
        .get_stats(route)
        .expect("stats should exist");
    assert!(
        stats.last_utility < 0.5,
        "utility should be reduced for high-latency route (got {})",
        stats.last_utility,
    );
}

#[then("the utility score should be penalized for cost")]
async fn then_utility_penalized_cost(world: &mut BddWorld) {
    let stats = world
        .bandit_policy
        .get_stats("expensive-route")
        .expect("expensive route stats should exist");
    assert!(
        stats.last_utility < 0.5,
        "utility should be penalized for cost (got {})",
        stats.last_utility,
    );
}

#[then("uncertain routes have a chance of selection")]
async fn then_uncertain_routes_chance(world: &mut BddWorld) {
    // Run selection many times and verify both known and unknown routes get selected
    let route_refs: Vec<&str> = world
        .bandit_routes
        .iter()
        .map(std::string::String::as_str)
        .collect();
    let mut selected_routes: HashSet<String> = HashSet::new();
    for _ in 0..100 {
        if let Some(selected) = world.bandit_policy.select_route(&route_refs) {
            selected_routes.insert(selected);
        }
    }
    assert!(
        selected_routes.len() >= 2,
        "Thompson sampling should explore multiple routes (selected {} out of {})",
        selected_routes.len(),
        route_refs.len(),
    );
}
...
async fn then_high_success_likely(world: &mut BddWorld) {
    let route_refs: Vec<&str> = world
        .bandit_routes
        .iter()
        .map(std::string::String::as_str)
    ...
    assert!(
        best_count > trials / 2,
        "best-route should win majority of selections (won {best_count}/{trials})",
    );
}

#[then("fallbacks should prefer different providers")]
async fn then_fallbacks_different_providers(world: &mut BddWorld) {
    // Verify that not all routes share the same provider prefix
    let providers: HashSet<&str> = world
        .bandit_routes
        .iter()
        .map(|r| r.split('-').next().unwrap_or("unknown"))
        .collect();
    assert!(
        providers.len() > 1,
        "fallbacks should include routes from different providers",
    );
}
