...
impl LoomRateLimiter {
    ...
    // ALLOW: Test-only — loom Mutex never poisons, and guard scope is
    // intentional (mirrors production RateLimiter lock pattern).
...
fn rate_limiter_two_threads_no_exceed() {
    loom::model(|| {
        ...
        // ALLOW: Test-only unwrap on loom thread handles.
...
fn rate_limiter_concurrent_prune_and_check() {
    loom::model(|| {
        ...
        let t2 = loom::thread::spawn(move || {
            // ALLOW: Test-only — loom Mutex never poisons, guard dropped
            // immediately after retain.
            #[allow(clippy::unwrap_used)]
            {
