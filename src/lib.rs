...
pub fn build_app_state(config: GatewayConfig, rate_limit: Option<u64>) -> AppState {
    ...
    let executor = std::sync::Arc::new(RouteExecutor::new(
        ExecutorConfig::default(),
        metrics,
        health,
    ));

    let classifier = std::sync::Arc::new(DefaultRequestClassifier);

    let collector = std::sync::Arc::new(MemoryTraceCollector::with_default_size());
    ...
    let rate_limiter = std::sync::Arc::new(RateLimiter::new(rate_limit));
