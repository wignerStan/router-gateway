...
use crate::routing::fallback::FallbackConfig;
use crate::routing::health::HealthManager;
use crate::routing::metrics::MetricsCollector;
...
impl Router {
    ...
    pub fn with_config(config: RouterConfig) -> Self {
        Self {
            candidate_builder: crate::routing::candidate::CandidateBuilder::new(),
            constraint_filter: crate::routing::filtering::ConstraintFilter::new(),
            utility_estimator: UtilityEstimator::new(),
            bandit_policy: Arc::new(Mutex::new(BanditPolicy::new())),
            selector: crate::routing::selector::SmartSelector::new(
                crate::routing::config::SmartRoutingConfig::default(),
            ),
            fallback_planner: crate::routing::fallback::FallbackPlanner::with_config(
                FallbackConfig {
    ...
    pub async fn plan(
        ...
        if self.config.enable_session_affinity {
            if let (Some(sid), Some(route)) = (session_id, &selected) {
                let _ = self
                    .session_manager
