...
impl HealthManager {
    ...
    ///
    /// ```
    /// # use crate::routing::{HealthManager, HealthConfig, HealthStatus};
    /// # #[tokio::main]
    /// # async fn main() {
    ...
    /// ```
    #[must_use]
    pub fn new(config: crate::routing::config::HealthConfig) -> Self {
        Self {
    ...
    #[must_use]
    pub fn with_limit(config: crate::routing::config::HealthConfig, max_entries: usize) -> Self {
        Self {
            health: Arc::new(tokio::sync::RwLock::new(HashMap::new())),
            config,
    ...
    pub fn set_config(&mut self, config: crate::routing::config::HealthConfig) {
        self.config = config;
    }
}
